...
describe('a fetched schema is bounded in size and in time', () => {
  ...
  it('stops an endless response instead of buffering it', async () => {
    ...
    try {
      ...
      // line was fine, so this is an unobtainable schema (exit 69), not a usage error.
      expect(code).toBe(69);
...
describe('an unobtainable schema (exit 69): a --schema no configured source would supply', () => {
  ...
  it('exits 69 and reports SCHEMA_NOT_FOUND, not a per-file usage error', async () => {
